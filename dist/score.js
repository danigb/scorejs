...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function(plugins) {
	   ...
	   * Public stuff
	   */
	  Score.parse = __webpack_require__(7);
	  Score.Time = Time;
	  Score.Event = Event;
	  ...
	  Score.fn.toString = function() {
	    return this.events.map(function(e) { return e.str(); }).join(' ');
	  }

	  Score.fn.each = function(callback) {
	...
	function parseList(events, list, position, duration) {
	  ...
	  list.forEach(function(item) {
	    if (Array.isArray(item)) {
	      ...
	      events.push(event(item, position, duration));
	    }
	    position += duration;
	  });
	}
